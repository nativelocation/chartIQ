...
const ThemeUI = (state = initialState, action) => {
	switch(action.type){
            ...
            let themeHelper = new window.CIQ.ThemeHelper({
                'stx': action.ciq
            })
			...
			case Types.CHANGE_THEME:
				if (action.theme.name.indexOf('+ New Theme') > -1) {
					...
					window.CIQ.localStorageSetItem('myChartCurrentThemeName', JSON.stringify(action.theme.name));
	    ...
	    case Types.SAVE_THEME:
						...
						item.className = window.$$$('body').className
				    ...
				    if((existsIndex > -1 && newThemeList[existsIndex].builtIn === true) || action.name==="+ New Theme") {
						...
						window.CIQ.localStorageSetItem('myChartThemes', JSON.stringify(newThemeList));
						window.CIQ.localStorageSetItem('myChartCurrentThemeName', JSON.stringify(action.name));
		...
		case Types.RESTORE_THEMES:
			let restoredThemeList = JSON.parse(window.CIQ.localStorage.getItem('myChartThemes')) || state.themeList;
			let restoredCurrentThemeName = JSON.parse(window.CIQ.localStorage.getItem('myChartCurrentThemeName')) || restoredThemeList[0].name;
...
function updateThemeSettings(themeHelper, currentSettings, newParams){
    ...
    rgbaColor = (newParams && newParams.color) ? window.CIQ.hexToRgba('#'+newParams.color) : null
...
function setTheme(themeHelper, theme) {
	if (theme.settings) {
		themeHelper.settings = window.CIQ.clone(theme.settings);
		themeHelper.update();
		window.$$$('body').className = theme.settings.className
	} else if (theme.builtIn === true) {
		window.$$$('body').className = theme.className
