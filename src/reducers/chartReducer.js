...
const chart = (state = initialState, action) => {
  switch (action.type) {
      ...
      let ciq = new window.CIQ.ChartEngine({
        container: action.container
      })
      ciq.attachQuoteFeed(window.quoteFeedSimulator, { refreshInterval: state.refreshInterval })
      ciq.setMarketFactory(window.CIQ.Market.Symbology.factory);
      let layout = window.CIQ.localStorage.getItem('myChartLayout');
      ...
      let preferences = window.CIQ.localStorage.getItem('myChartPreferences');
      if (preferences !== null){
    ...
    case Types.CHANGE_VECTOR_STYLE:
      ...
      if (type === "bold") {
        ...
        state.ciq.currentVectorParameters.currentColor = window.CIQ.hexToRgba('#' + action.style.color)
      } else if (type === "fillColor") {
        state.ciq.currentVectorParameters.fillColor = window.CIQ.hexToRgba('#' + action.style.color)
      } else return state
...
function restoreDrawings(stx){
	...
	var memory=window.CIQ.localStorage.getItem(stx.chart.symbol.toUpperCase());
	if(memory!==null){
