...
class MenuSelect extends React.Component{
	...
	render(){
		...
		let options = this.props.options.map((option, i) => {
			...
			let optionLabel = this.props.name ? option[this.props.name] : (this.props.labelNeedsTransform ? this.props.labelTransform(option) : option)
			let buttonCName = (this.state.selected && option === this.state.selected) ? 'ciq-checkbox ciq-active' : 'ciq-checkbox'
			let select = this.state.hasCheckboxes ? this.selectOption.bind(this, {}, option, true) : onSelect;
