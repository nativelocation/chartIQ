...
export function undo(){
    return (dispatch, getState) => {
        ...
        if (undone){
            let drawings = window.CIQ.shallowClone(undone),
            oldDrawings = window.CIQ.shallowClone(state.chart.ciq.drawingObjects);
...
export function redo(){
    return (dispatch, getState) => {
        ...
        if (redone){
            let drawings = window.CIQ.shallowClone(redone),
            oldDrawings = window.CIQ.shallowClone(state.chart.ciq.drawingObjects);
            state.chart.ciq.drawingObjects=drawings;
            return Promise.all([
...
export function changeDrawings(params){
    return (dispatch, getState) => {
        ...
        if(tmp.length===0){
            window.CIQ.localStorage.removeItem(state.chart.symbol);
        }else{
            window.CIQ.localStorageSetItem(state.chart.symbol, JSON.stringify(tmp));
		}
...
export function saveLayout(){
    return (dispatch, getState) => {
        let state = getState(),
                ...
                window.CIQ.localStorageSetItem("myChartLayout", savedLayout);
                window.CIQ.localStorageSetItem('myChartPreferences', JSON.stringify(state.chart.ciq.exportPreferences()));
    }
}
